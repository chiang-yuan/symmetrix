...
def extract_mace_data(model_path, species, head=None, num_spline_points=256):
    ...
    atomic_numbers = _parse_species(species)
...
def _check_compatibility(model):
...
def _extract_layer_0(
    ...
    logging.info("M0")
    _extract_M0(model, atomic_numbers, num_channels, l_max, output)


def _extract_M0(model, atomic_numbers, num_channels, l_max, output):
...
def _extract_layer_1(
    ...
    logging.info("Phi1")
    _extract_Phi1(model, l_max, L_max, num_channels, output)
    ...
    logging.info("M1")
    _extract_M1(model, atomic_numbers, num_channels, l_max, output)

    logging.info("H2")
    _extract_H2(model, atomic_numbers, num_channels, output)


def _extract_Phi1(model, l_max, L_max, num_channels, output):
    Phi1_l = [ir[1].l for ir in model.interactions[1].conv_tp.irreps_out]
